 ...
 * Attachment reading.
 *
 * Two routes share the same ownership rules (row + mail must belong to the
 * caller) but differ in how they are addressed:
 *
...
export async function serveAttachmentById(c) {
	...
	if (!user?.userId || !attId) return notFound();

	const attachment = await orm(c).select({ key: att.key, filename: att.filename, mimeType: att.mimeType })
		.from(att).innerJoin(email, eq(att.emailId, email.emailId)).where(
			and(eq(att.attId, attId), eq(att.userId, user.userId), eq(email.userId, user.userId))
		).get();
