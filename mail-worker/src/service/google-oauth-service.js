...
const googleOauthService = {
	...
	async authorizeUrl(c, state, nonce) {
	...
	async saveState(c, value) {
		...
		await c.env.kv.put(KvConst.OAUTH_GOOGLE_STATE + state, JSON.stringify({
			...value,
	...
	async startLogin(c) {
		...
		const state = await this.saveState(c, { intent: 'login', nonce });
		return this.authorizeUrl(c, state, nonce);
	},

	async startLink(c, userId, sessionToken) {
		if (!sessionToken) throw new BizError('Authentication is required');
		const nonce = randomToken();
		const state = await this.saveState(c, { intent: 'link', userId, sessionToken, nonce });
		return this.authorizeUrl(c, state, nonce);
	},

	async consumeState(c, state) {
		if (!state || state.length < 32) throw new BizError('Invalid OAuth state');
		const key = KvConst.OAUTH_GOOGLE_STATE + state;
		const value = await c.env.kv.get(key, { type: 'json' });
		await c.env.kv.delete(key);
		if (!value || value.expiresAt < Date.now() || !['login', 'link'].includes(value.intent) || !value.nonce) {
			throw new BizError('OAuth state has expired');
		}
		return value;
	},

	async exchangeCode(c, code) {
		...
		const response = await fetch('https://oauth2.googleapis.com/token', {
			...
			body: new URLSearchParams({ client_id: clientId, client_secret: clientSecret, code, redirect_uri: this.callbackUrl(c), grant_type: 'authorization_code' })
		});
	...
	async getGoogleUser(c, code, nonce) {
		const token = await this.exchangeCode(c, code);
		const claims = await this.verifyIdToken(c, token.id_token, nonce);
	...
	async handleCallback(c) {
		...
		try {
			...
			const googleUser = await this.getGoogleUser(c, c.req.query('code'), state.nonce);
			...
			await c.env.kv.put(KvConst.OAUTH_GOOGLE_COMPLETE + grant, JSON.stringify({ token }), { expirationTtl: COMPLETE_TTL_SECONDS });
	...
	async completeLogin(c, grant) {
		if (!grant || grant.length < 32) throw new BizError('Google sign-in has expired');
		const key = KvConst.OAUTH_GOOGLE_COMPLETE + grant;
		const result = await c.env.kv.get(key, { type: 'json' });
		await c.env.kv.delete(key);
		if (!result?.token) throw new BizError('Google sign-in has expired');
